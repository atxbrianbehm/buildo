...
**Status:** Milestone 6N opt-in run-controller remote material invocation foundation
...
This slice is still adapter foundation, not complete remote-material integration. The later 6C/6D/6E/6F/6G/6H/6I/6J/6K/6L slices add in-memory cache lookup/store, timeout fallback, bounded cache-miss concurrency, transient-failure retry, pure decoded-overlay compositing, a client-safe remote image artifact bridge, a browser PNG layer decoder, a schema-validated route client, a Vite dev route host, and a route-result application coordinator, but there is still no durable cache persistence, run-controller/UI invocation, or Atlas Lab revised-prompt diagnostics.
...
Focused tests verify that `requestHashFor` matches generated artifact hashes and that repeated route requests reuse the cached artifact without making a second OpenAI transport call. This is still a process-memory cache foundation only; the later 6D/6E/6F/6G/6H/6I/6J/6K/6L/6M/6N slices add timeout fallback, bounded cache-miss concurrency, transient-failure retry, pure overlay compositing, the image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, and opt-in run-controller handoff, but durable storage, TTL/eviction, in-flight request coalescing, user-facing app invocation, and Atlas Lab cache-status UI remain open.
...
Focused tests cover the timeout fallback diagnostic, upstream abort signal, secret non-echoing, and the no-cache-after-timeout invariant. The later 6E/6F/6G/6H/6I/6J/6K/6L/6M/6N slices add bounded cache-miss concurrency, transient-failure retry, pure decoded-overlay compositing, the remote image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, and opt-in run-controller handoff; durable cache persistence, cancellation progress events, user-facing app invocation, and Atlas Lab error surfacing remain open.
...
Focused tests verify that three uncached approved source requests run with two active OpenAI transports at a time and still return artifacts in request order. The later 6F/6G/6H/6I/6J/6K/6L/6M/6N slices add transient-failure retry, pure remote overlay compositing, the remote image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, and opt-in run-controller handoff; durable cache persistence, in-flight request coalescing, user-facing app invocation, and Atlas Lab provider-progress surfacing remain open.
...
Focused tests verify that a first transport failure containing a test secret is retried once, succeeds on the second call, returns generated `openai-image` output, and does not echo the API key. This slice does not add exponential backoff, retry-after parsing, durable cache persistence, in-flight request coalescing, user-facing app invocation, Atlas Lab provider-progress surfacing, or remote overlay compositing. Milestone 6G adds the first pure compositing utility after this route-layer retry foundation.
...
Focused tests cover middleware registration, request forwarding, status/header/body propagation, and fallback diagnostics from the hosted route. This slice does not wire the app/run controller to call the route client, does not decode or composite returned images in an app run, does not surface revised prompts in Atlas Lab, and does not add durable remote cache persistence.
...
Focused tests cover generated remote overlay compositing over procedural sources, preservation of procedural height/roughness channels, route summary and revised-prompt summary capture, procedural fallback diagnostics, no decode attempt during fallback, and missing-generated-artifact diagnostics. This slice does not wire `BuildingRunController` to call the coordinator, does not add UI controls/feature flags, does not persist remote cache entries, and does not surface revised prompts in Atlas Lab yet.
...
Focused fixture coverage injects a mocked generated remote response for `source.wall.primary`, verifies that the route call receives only the selected remote source, decodes the returned artifact, changes the packed atlas content hash, preserves the compiled `RuntimeBuildingIR` graph hash and metrics, and captures the remote provider/revised-prompt summary. This slice does not turn remote material generation on from the default app shell, does not add a user feature flag, and does not surface revised prompts in Atlas Lab UI.
...
```text
typecheck: passed
unit tests: passed, 140 tests across 40 files
lint: passed
build: passed
...
Assembly fixture remote-material integration focused test stack: passed, 22 tests across 5 files
Remote application coordinator/client/bridge/compositor focused tests: passed, 14 tests across 4 files
Remote route client/bridge/decoder focused tests: passed, 11 tests across 3 files
Vite material-provider route host focused test: passed
Building state/controller focused tests: passed, 12 tests in 1 file
