...
**Status:** Milestone 6P user-facing remote material feature flag foundation
...
Focused tests verify that `requestHashFor` matches generated artifact hashes and that repeated route requests reuse the cached artifact without making a second OpenAI transport call. This is still a process-memory cache foundation only; the later 6D/6E/6F/6G/6H/6I/6J/6K/6L/6M/6N/6O/6P slices add timeout fallback, bounded cache-miss concurrency, transient-failure retry, pure overlay compositing, the image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, opt-in run-controller handoff, Atlas Lab remote detail surfacing, and a user-facing feature flag, but durable storage, TTL/eviction, and in-flight request coalescing remain open.
...
Focused tests cover the timeout fallback diagnostic, upstream abort signal, secret non-echoing, and the no-cache-after-timeout invariant. The later 6E/6F/6G/6H/6I/6J/6K/6L/6M/6N/6O/6P slices add bounded cache-miss concurrency, transient-failure retry, pure decoded-overlay compositing, the remote image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, opt-in run-controller handoff, Atlas Lab remote detail surfacing, and a user-facing feature flag; durable cache persistence and cancellation progress events remain open.
...
Focused tests verify that three uncached approved source requests run with two active OpenAI transports at a time and still return artifacts in request order. The later 6F/6G/6H/6I/6J/6K/6L/6M/6N/6O/6P slices add transient-failure retry, pure remote overlay compositing, the remote image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, opt-in run-controller handoff, Atlas Lab remote detail surfacing, and a user-facing feature flag; durable cache persistence, in-flight request coalescing, and Atlas Lab provider-progress surfacing remain open.
...
Focused tests verify that a first transport failure containing a test secret is retried once, succeeds on the second call, returns generated `openai-image` output, and does not echo the API key. This slice does not add exponential backoff, retry-after parsing, durable cache persistence, in-flight request coalescing, Atlas Lab provider-progress surfacing, or remote overlay compositing. Milestone 6G adds the first pure compositing utility after this route-layer retry foundation.
...
Focused coverage verifies the default-off prompt state, material-source invalidation preview, controller gating when a remote adapter is configured, and the Prompt Lab checkbox interaction. This is a feature-flag foundation; durable cache persistence, provider-level cancellation progress, and a fully exercised provider-aware e2e path remain later work.
...
```text
typecheck: passed
unit tests: passed, 144 tests across 40 files
lint: passed
build: passed
