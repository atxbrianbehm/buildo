...
**Status:** Milestone 8 Slice 2–8 through visual QA packet export (M1–M11; Phase C complete)
...
**Post-MVP kit → WFC → UTDG → block plan:** `docs/plans/kit-grammar-wfc-utdg-block.md`
...
The app currently contains the deterministic domain, atlas, component, compiler, renderer, state, four-room UI plus sample gallery, remote-material, route document id, provider-aware smoke, and Milestone 7A-7X persistence/export/performance foundations. Browser-side serialized artifact persistence now has a tested IndexedDB boundary, a schema-versioned completed-family packet/cache-entry adapter, a run-controller handoff to an injected completed-family persistence writer, default app wiring that writes completed-family records to IndexedDB when available, a controller read-restore seam that can select the latest cached completed family for a route document id, a packet-to-runtime fixture restore adapter that can inflate persisted packets into live Assembly Hall fixtures, app startup wiring that tries cached restore before fresh generation, a portable completed-family export bundle builder, an app download action that exports the active completed-family bundle as JSON, a pure export verifier that decodes bundle atlas PNGs and recompiles the exported building graph, an import adapter that turns portable export JSON back into a runtime-restorable completed-family packet, an app import action that restores a downloaded bundle into the live runtime, a pure 100-building shared-family benchmark scene/report builder, compiler-level high/low detail switching that can omit decorative trim/cornice batches while preserving default high-detail output, a Prompt Lab detail-level selector that runs low-detail geometry without regenerating material artifacts, an Assembly Hall benchmark report action that surfaces the 100-building report in the app, Assembly Hall renderer compatibility diagnostics for unsupported WebGPU/WebGL paths, a benchmark documentation artifact that labels measured, estimated, and not-captured profile fields plus known limitations, cooperative benchmark yielding/cancellation so the 100-building benchmark can pause between batches and abort stale work, a renderer-sampled 16-building orbit benchmark surface, a schema-versioned Milestone 7 benchmark profile packet that combines construction/orbit reports with browser hardware context, an Assembly Hall download action that saves that packet as JSON evidence, a sanitized remote-material proof packet that Atlas Lab can download when the route reports generated OpenAI material output, a Sample Gallery room that exposes eight generated family-variant previews as HTML/SVG cards with seed, metric, family, and atlas lineage, an initial renderer-independent art-kit contract layer with a `late19cApartmentKit` fixture plus validation for module ids, sockets, material roles, meter bounds, and low-detail fallback references, and a visual-truth presentation prerequisite that remaps copied renderer geometry UVs into semantic atlas slots while preserving shared texture identity and immutable compiler IR, frames the front facade, configures renderer color/tone/shadows and ground contact, and provides WebGPU-safe textured/clay inspection, plus art-kit snap-grid helpers and a seeded-greedy facade module planner that emits deterministic diagnostic-backed front/rear/side placement plans into the building graph through a renderer-independent Group node, a `ModuleInstanceSet` expander, compiler kit-mode opening placement from those instances, explicit Prompt Lab `fidelityMode` (`kit` default / `proof` legacy) with graph omission of the art-kit plan node in proof mode, Artifact Trace surfacing of art-kit plan summaries/diagnostics, Component Forge window/cornice family swap controls that recompile without material regeneration, high-detail profiled cornice/belt/roof-cap/quoin geometry, high-fidelity window/door assemblies (recess, frame, mullions, sill/lintel, arched crown steps, door leaf/transom) with paired glass instance batches and renderer compound recipe geometry, art-kit material set bindings that apply `metersPerTile` / proceduralSource / glTF channel hints onto atlas slots and scale-aware procedural brick generation, fidelity mode recorded on completed-family persistence/export packets and benchmark profile packets, an Art Kit Lab room (`#room=artKitLab`) for module/material inspection with clay/wireframe/textured previews plus Sample Gallery fidelity labels, and Assembly Hall export of a schema-versioned visual QA packet with measured/estimated/not-captured checklist evidence; configured live-provider capture, externally captured representative-machine sign-off, WFC facade solver, and plan-driven wall meshes remain later work.
...
## 6.62 Family Benchmark Scene Foundation

Actual Milestone 7M benchmark-scene paths:

```text
src/features/building-family/performance/familyBenchmarkScene.ts
src/features/building-family/tests/familyBenchmarkScene.test.ts
docs/architecture/dynamic-building-family-integration.md
```

`createFamilyBenchmarkScene()` creates a new shared `BuildingFamilyRuntime` from an existing completed `AssemblyHallFixture`, compiles 100 benchmark building IRs from the same family spec/catalog/graph with distinct benchmark building ids, mounts every runtime under one family root, and returns a schema-versioned report. The report records per-building and aggregate draw/instance/triangle metrics, a runtime-IR transfer byte estimate, injected-clock compile and mount timing totals, shared atlas/material metrics, and the current 150,000-triangle and family-asset-sharing target checks.
...
## 6.64 App Detail-Level Control

Actual Milestone 7O app detail-control paths:
...
Prompt Lab now exposes a `Detail Level` select with `High` and `Low` values. The control is stored with the committed prompt controls, participates in invalidation as a geometry-only change, and keeps material sources, packed atlas, and component catalog reusable. `createAssemblyHallFixture()` passes the selected detail level into the compiler for both the active runtime IR and the 16-variant stress summary, and Prompt Trace records the requested detail level.

The run-controller fixture artifact id now includes the runtime IR source hash so high- and low-detail reruns for the same building/atlas do not collide in the artifact registry. The compiler keeps high-detail source hashes backward-compatible while low-detail runtime IR hashes include the detail mode. Focused App coverage first failed on the missing control, then caught the registry collision as a failed rerun; the final coverage switches to low detail, verifies geometry-only invalidation, reruns, and confirms Assembly Hall metrics/stage visibility reflect the low-detail runtime. App-facing benchmark UI, measured results, compatibility diagnostics, and hardening remain later Milestone 7 work.

## 6.65 App Benchmark Report Surface

Actual Milestone 7P benchmark-report paths:
...
Assembly Hall now exposes a `100-Building Benchmark` performance panel backed by the existing `createFamilyBenchmarkScene()` foundation. The panel starts from an explicit not-run state, runs the shared-family 100-building benchmark on demand, captures the schema-versioned report, immediately disposes the temporary benchmark family runtime, and displays the benchmark kind, building count, aggregate draw/instance/triangle metrics, runtime IR transfer estimate, atlas content hash, per-building triangle count, compile and mount timings, and target checks for the 150,000-triangle budget plus family asset sharing.

The benchmark action uses run ids and an unmount guard so fixture changes or navigation do not publish stale benchmark results back into the current Assembly Hall view. Focused Assembly Hall coverage first failed on the missing report surface, then runs the real benchmark through the UI action and verifies the report kind, 100-building count, atlas identity, per/aggregate triangle metrics, and target-check labels. Measured benchmark result capture/documentation, compatibility diagnostics, performance hardening, and the interactive 16-building orbit proof remain later Milestone 7 work.

## 6.66 Compatibility Diagnostics Surface

Actual Milestone 7Q compatibility-diagnostics paths:
...
Assembly Hall now derives a schema-versioned renderer compatibility report from the fixture backend support, the active renderer backend, renderer fallback reason, and any activation error. The report records Three.js revision, preferred and active backend, WebGL/WebGPU availability, fallback availability/usage, and diagnostics with stable codes for no backend, unavailable WebGPU, unavailable WebGL, WebGPU fallback, activation failure, or a ready informational state.
...
`createFamilyBenchmarkScene()` now accepts an optional `AbortSignal`, configurable `yieldEvery` interval, and injectable `yieldToMainThread()` hook. The 100-building benchmark validates the yield interval, checks cancellation before and after compile/mount work, yields between compile and mount batches by default, and disposes the temporary family runtime if cancellation or another error interrupts benchmark generation before a report is returned.

Assembly Hall now owns an `AbortController` per benchmark run. Starting a new benchmark, changing fixtures, or unmounting Assembly Hall aborts the previous benchmark signal and prevents stale benchmark work from publishing into the current view. Focused coverage first failed on missing yield calls, missing abort rejection, and missing signal propagation; the final tests verify cooperative batch yields, abort-before-publish behavior, and unmount-driven benchmark cancellation. Milestone 7T later adds the interactive 16-building orbit proof; broader browser/hardware profile capture remains separate.

## 6.69 16-Building Orbit Benchmark

Actual Milestone 7T orbit-benchmark paths:

```text
src/features/building-family/performance/familyOrbitBenchmarkScene.ts
src/features/building-family/tests/familyOrbitBenchmarkScene.test.ts
src/features/building-family/performance/familyBenchmarkDocumentation.ts
src/features/building-family/tests/familyBenchmarkDocumentation.test.ts
src/features/building-family/ui/AssemblyHall.tsx
src/features/building-family/tests/AssemblyHall.test.tsx
...
`createFamilyOrbitBenchmarkScene()` creates a separate shared-family runtime with sixteen generated building runtimes arranged in a 4-by-4 orbit layout. It activates the same renderer factory used by Assembly Hall, samples a deterministic camera orbit around the family root, renders each sample, records average, p95, and max frame times, checks a 33.4 ms interactive-frame budget, and verifies family assets remain shared across all sixteen buildings. Abort handling disposes the temporary family runtime and renderer if generation or frame sampling is cancelled.

Assembly Hall now exposes a `16-Building Orbit Benchmark` performance panel beside the existing 16-variant stress summary and 100-building construction benchmark. The panel runs the orbit benchmark on demand, disposes the temporary orbit runtime after reading the report, and displays report kind, building count, renderer backend, p95/average/max frame timing, frame budget, atlas lineage, aggregate triangles, and target checks for interactive orbit and family asset sharing. The 100-building benchmark documentation now points frame-time readers to this separate orbit benchmark instead of saying the orbit proof is missing. Focused coverage first failed on the missing module/surface and stale limitation text, then verifies renderer-sampled frame metrics, abort cleanup, Assembly Hall report surfacing, and corrected 100-building benchmark documentation. Broader representative-machine profiling and configured live-provider proof remain later work.

## 6.70 Benchmark Profile Packet

Actual Milestone 7U benchmark-profile-packet paths:

```text
src/features/building-family/performance/familyBenchmarkProfilePacket.ts
src/features/building-family/tests/familyBenchmarkProfilePacket.test.ts
src/features/building-family/ui/AssemblyHall.tsx
src/features/building-family/tests/AssemblyHall.test.tsx
docs/architecture/dynamic-building-family-integration.md
```

`createFamilyBenchmarkProfilePacket()` now creates a schema-versioned `dynamic-building-family-milestone-7-profile` artifact from the 100-building construction report and the 16-building orbit report. The packet preserves both reports, captures browser/runtime context such as user agent, hardware concurrency, device memory when exposed, device pixel ratio, viewport, Three.js revision, renderer backend availability, and orbit backend, then normalizes Milestone 7 profile coverage across CPU compile time, runtime mount time, worker transfer estimate, GPU memory, triangles, instances, draw calls, and frame time.

The packet keeps GPU memory explicitly `not-captured` because no stable browser GPU memory API is used, marks worker transfer size as estimated, and marks frame time as measured from the orbit benchmark p95. Assembly Hall now renders a `Milestone 7 Profile Packet` section once both benchmark reports are present, with packet identity, captured environment, coverage rows, target checks, and limitations. Focused coverage first failed on the missing module and missing UI section, then verifies JSON round-trip validation, invalid-packet rejection, preserved orbit samples, profile metric statuses, local limitations, and Assembly Hall surfacing after both benchmarks run. A representative-machine sign-off still requires rerunning this packet on the target hardware and preserving that resulting evidence outside this local automated slice.

## 6.71 Benchmark Profile Packet Export

Actual Milestone 7V benchmark-profile-packet-export paths:
...
Assembly Hall now shows a `Download Profile Packet` action after the 100-building construction benchmark and 16-building orbit benchmark have both produced the schema-versioned Milestone 7 profile packet. The action writes the exact packet JSON to `<familyId>-benchmark-profile-packet.json` through the same browser Blob/Object URL pattern used by the completed-family export lane, preserving the captured environment, raw reports, profile coverage, target checks, and limitations for representative-machine evidence review. Focused coverage first failed on the missing button, then verifies the downloaded JSON payload, file name, application/json blob type, anchor click, and object URL revocation.

## 6.72 Remote Material Proof Packet

Actual Milestone 7W remote-material-proof-packet paths:

```text
src/features/building-family/materials/remoteMaterialProofPacket.ts
src/features/building-family/tests/remoteMaterialProofPacket.test.ts
src/features/building-family/ui/AtlasLab.tsx
src/features/building-family/tests/atlasDebugExport.test.tsx
src/app/App.css
docs/architecture/dynamic-building-family-integration.md
...
`createRemoteMaterialProofPacket()` creates a schema-versioned `dynamic-building-family-remote-material-proof` packet from a generated remote-material route summary, the applied remote source hashes, and the current atlas/debug-export identity. The packet records the generated route status, provider id, request hash, accepted request count, cache state, remote source request/content hashes, revised prompt counts, sanitized diagnostics, and known limitations while intentionally omitting provider secrets and raw remote image bytes.
...
App routing/document-id/IndexedDB-write/reload-restore/export-download/export-import/detail-level/committed-rerun/cancel/remote-toggle/cache-selector focused tests: passed, 14 tests in 1 file
