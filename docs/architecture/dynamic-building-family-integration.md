...
**Status:** Milestone 6O Atlas Lab remote material detail surfacing foundation
...
Focused tests verify that `requestHashFor` matches generated artifact hashes and that repeated route requests reuse the cached artifact without making a second OpenAI transport call. This is still a process-memory cache foundation only; the later 6D/6E/6F/6G/6H/6I/6J/6K/6L/6M/6N/6O slices add timeout fallback, bounded cache-miss concurrency, transient-failure retry, pure overlay compositing, the image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, opt-in run-controller handoff, and Atlas Lab remote detail surfacing, but durable storage, TTL/eviction, in-flight request coalescing, and user-facing app invocation remain open.
...
Focused tests cover the timeout fallback diagnostic, upstream abort signal, secret non-echoing, and the no-cache-after-timeout invariant. The later 6E/6F/6G/6H/6I/6J/6K/6L/6M/6N/6O slices add bounded cache-miss concurrency, transient-failure retry, pure decoded-overlay compositing, the remote image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, opt-in run-controller handoff, and Atlas Lab remote detail surfacing; durable cache persistence, cancellation progress events, and user-facing app invocation remain open.
...
Focused tests verify that three uncached approved source requests run with two active OpenAI transports at a time and still return artifacts in request order. The later 6F/6G/6H/6I/6J/6K/6L/6M/6N/6O slices add transient-failure retry, pure remote overlay compositing, the remote image artifact bridge, browser PNG layer decoding, the route client, the Vite dev route host, coordinator/fixture integration, opt-in run-controller handoff, and Atlas Lab remote detail surfacing; durable cache persistence, in-flight request coalescing, user-facing app invocation, and Atlas Lab provider-progress surfacing remain open.
...
Focused fixture coverage injects a mocked generated remote response for `source.wall.primary`, verifies that the route call receives only the selected remote source, decodes the returned artifact, changes the packed atlas content hash, preserves the compiled `RuntimeBuildingIR` graph hash and metrics, and captures the remote provider/revised-prompt summary. This slice does not turn remote material generation on from the default app shell and does not add a user feature flag. Later 6O work surfaces the captured summary in Atlas Lab.
...
The root app passes `fixture.remoteMaterialApplication` through to Atlas Lab, but the default app path remains procedural-only because no user-facing feature flag supplies the remote lane yet.
...
```text
typecheck: passed
unit tests: passed, 141 tests across 40 files
lint: passed
build: passed
